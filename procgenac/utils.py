...
class Storage:
    ...
    def get_generator(self, batch_size=1024):
        iterator = BatchSampler(
            SubsetRandomSampler(range(self.num_steps * self.num_envs)), batch_size, drop_last=True
        )
...
def get_formatter():
    def human_format(num, pos):
        ...
        if num.is_integer():
            tick = f"{num:.0f}{prefix}"
        else:
            tick = f"{num:.2f}{prefix}"
        return tick

    return FuncFormatter(human_format)


def save_rewards(steps, rewards, filepath):
    if not os.path.isdir(os.path.dirname(filepath)):
        os.makedirs(os.path.dirname(filepath))
...
def clear_mpi_env_vars():
    """
    from mpi4py import MPI will call MPI_Init by default.  If the child process has MPI environment
    variables, MPI will think that the child process is an MPI process just like the parent and
    do bad things such as hang.
    This context manager is a hacky way to clear those environment variables temporarily such as
    when we are starting multiprocessing
    Processes.
    """
    removed_environment = {}
